...
import { EmptyInputError, DuplicateKeyError, ParsingError } from './errors.js';
import { toString, normalizeLineEndings, filterUppercaseKeys, partitionKeys, decodeBase64, } from './utils.js';
/**
...
class VaultSecretParserImpl {
    ...
    parse(input, file_formatter) {
        ...
        const uppercaseMap = this.options.ignoreNonUppercase
            ? filterUppercaseKeys(rootMap)
            : rootMap;
        ...
        for (const [fileKey, base64Value] of Object.entries(fileKeys)) {
            try {
                ...
                const formattedValue = file_formatter
                    ? file_formatter(fileKey, base64Value)
                    : base64Value;
                // Decode base64
