...
function countRoutes(routesString) {
    ...
    if (isTreeFormat) {
        ...
        return lines.filter((line) => line.trim().startsWith('└──') || line.trim().startsWith('├──')).length;
    }
    else {
...
const routeLoggerPlugin = async (fastify, options) => {
    ...
    fastify.addHook('onReady', async function () {
        ...
        try {
            ...
            const hookDuration = Date.now() - hookStartTime;
            fastify.log.error({ error, stack: error instanceof Error ? error.stack : undefined, hook: 'onReady', plugin: 'core-route-logger', duration: hookDuration }, 'Route logger onReady hook failed');
