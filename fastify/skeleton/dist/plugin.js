...
import { AppRegistry, } from "./lib/app-registry.js";
const swaggerRegisterPlugin = async (fastify) => {
    // ==========================================
...
export default async (fastify) => {
    ...
    fastify.get("/sys/status/apps/:name", async (request, reply) => {
    ...
    fastify.get("/sys/status/apps/:name/health", async (request, reply) => {
        const app = fastify.apps.get(request.params.name);
        if (!app) {
            return reply.code(404).send({ error: "App not found" });
        }
